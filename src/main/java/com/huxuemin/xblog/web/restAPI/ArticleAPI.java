...
public class ArticleAPI {
    ...
    @RequestMapping(value = "/{articleId}/discuss/canreply", method = RequestMethod.GET, produces = {
            "application/json", "application/xml" })
    public ResponseEntity<String> canDiscuss(@PathVariable long articleId, HttpServletRequest request) {
        HttpSession session = request.getSession();
        String loginId = (String) session.getAttribute(SessionConstant.USERNAME);
        String password = (String) session.getAttribute(SessionConstant.PASSWORD);

        if (articleService.canReply(articleId, loginId, password)) {
            return new ResponseEntity<String>("{\"canreply\":\"true\"}", HttpStatus.OK);
        }
        throw new AuthException(AuthConstant.PUBLIC_DISCUSS);
    }

    @RequestMapping(value = "/{articleId}/discuss/open", method = RequestMethod.PUT, produces = { "application/json",
            "application/xml" })
    public ResponseEntity<String> openDiscuss(@PathVariable long articleId, HttpServletRequest request) {
        HttpSession session = request.getSession();
        String loginId = (String) session.getAttribute(SessionConstant.USERNAME);
        String password = (String) session.getAttribute(SessionConstant.PASSWORD);

        articleService.openDiscuss(articleId, loginId, password);
        return new ResponseEntity<String>("{\"open\":\"true\"}", HttpStatus.OK);
    }

    @RequestMapping(value = "/{articleId}/discuss/close", method = RequestMethod.PUT, produces = { "application/json",
            "application/xml" })
    public ResponseEntity<String> closeDiscuss(@PathVariable long articleId, HttpServletRequest request) {
        HttpSession session = request.getSession();
        String loginId = (String) session.getAttribute(SessionConstant.USERNAME);
        String password = (String) session.getAttribute(SessionConstant.PASSWORD);

        articleService.closeDiscuss(articleId, loginId, password);
        return new ResponseEntity<String>("{\"close\":\"true\"}", HttpStatus.OK);
    }
